---
title: 《树莓派不吃灰》第十八期：代理Windows台式机支持Remote Desktop外网远程桌面连接, 随时玩转Stable Diffusion WebUI / 《Use Pi》 Issue 18 Proxying Windows Desktop to Support Remote Desktop Connection from the Internet, Playing with Stable Diffusion WebUI anytime
categories:
- 树莓派不吃灰 / Use Pi
---
...
## 打开Windows台式机的远程连接服务，使其可以被远程连接 / Enable Remote Connection Service on Windows Desktop to Allow Remote Connection
...
🌈Through the above two pictures, we can get the computer name and username that will be used for subsequent login.



注意，当前的 Windows 用户，须支持密码登录;

🌈Note that the current Windows user must support password login;

## 通过路由器为Windows台式机分配固定IP  / Assign a Fixed IP to the Windows Desktop through the Router
...
🌈My desktop's IP in the LAN is `192.168.50.136`. I bind its MAC address to the IP, so the desktop will use IP `192.168.50.136` every time it connects to the internet.

## 设置树莓派端口转发 / Setting Up Port Forwarding on Raspberry Pi
...
🌈I create a stream.conf.d folder on the Nginx proxy server of my Raspberry Pi.
...
🌈And add a configuration file `/etc/nginx/stream.conf.d/192.168.50.136-3389-win-remote-desktop-connection.conf`
...
🌈Add stream related configuration in `/etc/nginx/nginx.conf`
...
🌈After the Nginx of the Raspberry Pi restarts successfully, accessing 192.168.50.10:3389 is equivalent to accessing 192.168.50.136:3389

## 通过frp设置公网访问 / Setting Up Public Network Access through frp


在树莓派的 frpc.ini中添加配置

🌈Add configuration in the frpc.ini of Raspberry Pi
...
🌈The meaning of this configuration is to forward the requests received by the server's 33891 port to the Raspberry Pi's 3389 port.

树莓派会通过 Nginx将 3389 端口的请求转发到 Windows 台式机。

🌈Raspberry Pi will forward the requests of 3389 port to Windows desktop through Nginx.


## 外网远程连接Windows台式机 / Remote Connection to Windows Desktop from the Internet

### Windows发起远程桌面连接 / Initiating Remote Desktop Connection from Windows
...
### macOS发起远程桌面连接 / Initiating Remote Desktop Connection from macOS
...
## Stable Diffusion WebUI 远程生产图片，让老显卡重新发光发热! / Stable Diffusion WebUI Produces Images Remotely, Making the Old Graphics Card Glow and Heat Up Again!

完成了本文的配置，我们就可以随时随地启动家里台式机的Stable Diffusion WebUI， 产出需要的图片了，make RTX2060 great again!

🌈After completing the configuration in this article, we can start the Stable Diffusion WebUI of the desktop at home anytime and anywhere, and produce the pictures we need, make RTX2060 great again!
...
🌈Although the Raspberry Pi has limited performance, it is very suitable for acting as a gateway. According to the method in this article, we can map powerful desktop devices to the public network for us to call via the Internet at any time.

最近翻某宝，发现树莓派4B价格也正在回落，内存8GB的板子已经到了500RMB的价位段（树莓派4B 8GB原价是75美刀），目测今年秋季能回归正常价格。

🌈Recently, I found that the price of Raspberry Pi 4B is also falling. The 8GB memory board has reached the price segment of 500 RMB (the original price of Raspberry Pi 4B 8GB is 75 US dollars). It is expected to return to normal price this autumn.

今天是2023年6月22日，祝各位读者端午节吃好喝好！

🌈Today is June 22, 2023, wishing all readers to eat and drink well on the Dragon Boat Festival!
